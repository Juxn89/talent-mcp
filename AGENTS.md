...
| `reject_candidate` | `talent.candidates.reject` | Destructive operation requiring confirmation via **MRTR** (`InputRequiredException` + `requestState`), including the degraded path — which is gated on `IsMrtrSupported` **and** `ClientCapabilities.Elicitation`, not on `IsMrtrSupported` alone; see the pitfall below |
...
19. ❌ **Guarding an elicitation on `IsMrtrSupported` alone** — a 2025-11-25 client has it `true`, and raising an elicitation anyway fails inside the SDK with `InvalidOperationException: Client does not support elicitation requests.`, which reaches the caller as `isError` with the message *stripped*. The guard is `IsMrtrSupported && ClientCapabilities?.Elicitation is not null`: the first asks whether the mechanism exists, the second whether anyone is there to answer.
20. ❌ **Asserting only `IsError` in a negative test** — only `McpException` keeps its message; every other exception, the SDK's own argument-binding failure included, becomes `"An error occurred invoking '<tool>'."`. So a call that never reached the code under test looks identical to one that refused for the right reason. Assert the message text. This already made one test pass for the wrong reason.
21. ❌ **Assuming `Mcp-Name` is always a tool name** — the header must equal whatever the body's own identifying field is for that JSON-RPC method: `name` for `tools/call`, but `taskId` for `tasks/get` (and `tasks/cancel`). The SDK reports the exact mismatch, but only a hand-rolled request hits this — the SDK client always sets it correctly.
...
27. ❌ **Enumerating trim/AOT sites from a written list instead of the analyzer** — the F6 inventory was three sites short, and the two it missed (`ToolExecutionTelemetry.cs:68` and `:138`) were in a file the list already named. Turn `IsAotCompatible` on and read the build output; anything else is a guess. Related: a trimmed publish over EF Core needs `-p:SuppressTrimAnalysisWarnings=true` **on the publish command line only** — ILLink warnings are MSBuild warnings and `TreatWarningsAsErrors` turns them into a publish that simply cannot complete. Archive the unsuppressed list; suppressing it quietly is the thing ADR-0002 warns about. See [ADR-0007](./docs/adr/0007-trim-clean-over-native-aot.md).
28. ❌ **Putting assembly version properties in `Directory.Packages.props`** — it is imported *after* `Directory.Build.props`, so a `<Version>` there silently wins and the one in `Directory.Build.props` is dead. That is not hypothetical: it carried 1.0.1 while the other said 1.0.0, and v1.0.2 through v1.0.5 all shipped stamped 1.0.1. Assembly versions go in `Directory.Build.props` or the `.csproj`; `Directory.Packages.props` pins package dependency versions and nothing else. Same import-ordering trap as `PackageReference Update=` (NU1015).
29. ❌ **Treating a clean trim analysis as evidence the trimmed build works** — measured 10 Sep 2026 in CI: `PublishTrimmed` produced exactly two `IL2026` diagnostics, both EF Core, and the resulting host **threw at startup on something neither of them named** — `NotSupportedException: JsonTypeInfo metadata for type 'Talent.Domain.Enums.WorkArrangement' was not provided`, from `AIFunctionFactory.ReflectionAIFunctionDescriptor.GetParameterMarshaller`. The SDK builds every tool's schema by reflecting over its parameter types, so each one needs `JsonTypeInfo` — and the domain enums in tool signatures have none once trimmed. Same lesson as ADR-0002's config A, on a different mechanism: static analysis and runtime disagree in *direction*, so only running the binary against the functional gate settles it. A publish size is not a result. See [ADR-0007](./docs/adr/0007-trim-clean-over-native-aot.md).
30. ❌ **Silencing `IL2026`/`IL3050` with a `#pragma` in a library that ships to NuGet** — a pragma suppresses the warning for **consumers trimming their own applications**, not only for this build. Measured 8 Sep 2026: a downstream trimmed publish reported 4 `HandleCodec` diagnostics with the pragmas removed and 0 with them in place. Combined with `IsTrimmable`, that publishes a safety guarantee while hiding the evidence against it. Use `[RequiresUnreferencedCode]`/`[RequiresDynamicCode]` instead, which warn the caller. And when the annotation would propagate somewhere unacceptable, the answer is usually a **new clean overload** beside the annotated old one, not a suppression — that also turns what looks like a breaking change into a minor. See [ADR-0007](./docs/adr/0007-trim-clean-over-native-aot.md).
31. ❌ **Assuming a pushed branch ran CI** — `.github/workflows/ci.yml` filters `push` by branch prefix, so a branch outside that list runs nothing and reports nothing. Hit on 10 Sep 2026: `chore/package-quality` was pushed, `gh run watch` was pointed at the run it produced, and there was no run — the watch exited 0 on an empty id, which reads exactly like success. The prefix list now covers the whole commit convention, but it drifts by construction: **add a prefix to the convention, add it to the workflow.** And when checking CI, confirm a run exists before trusting its colour.
32. ❌ **Shipping `appsettings.Development.json` in a package or image** — `PackAsTool` packs the publish output and the Web SDK publishes environment configs by default, so a glob of `appsettings*.json` put the dev handle signing key inside the tool package on nuget.org *and* the container image. That file's own comment says any client able to read it can forge a handle. Keep it in build output for `dotnet run` with `CopyToPublishDirectory="Never"`, never in publish output. `publish.yml` now refuses to push a package containing one, because publishing cannot be undone.
33. ❌ **Writing `<NoWarn>$(NoWarn)</NoWarn>` and believing it enforces anything** — it assigns the property to itself. It sat in `Talent.Mcp.Toolkit.csproj` under a comment about `ConfigureAwait`, so it read as enforcement of pitfall 10 and enforced nothing; CA2007 was not enabled anywhere. Same family as the false-greens above: a line that looks like a guard is not a guard. When adding one, break something on purpose and watch it fail.
