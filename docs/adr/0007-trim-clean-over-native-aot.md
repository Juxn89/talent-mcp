...
System.NotSupportedException: JsonTypeInfo metadata for type 'Talent.Domain.Enums.WorkArrangement'
was not provided by TypeInfoResolver of type
'[ModelContextProtocol.McpJsonUtilities+JsonContext, Microsoft.Extensions.AI.AIJsonUtilities+JsonContext]'
   at Microsoft.Extensions.AI.AIFunctionFactory.ReflectionAIFunctionDescriptor.GetParameterMarshaller
```

### What actually broke, and why it is the most useful thing here

Not `HandleCodec`, which this ADR named as the likeliest casualty. **Tool schema generation.** The SDK
builds each tool's descriptor by reflecting over its method parameters and asking the serializer for
`JsonTypeInfo` per parameter type. `WorkArrangement` is a domain enum used as a `search_jobs`
parameter; under trimming the source-generated resolvers have no contract for it, and the host throws
before the transport is up.

`AGENTS.md` had already flagged the shape of this — *"`SkillCategory` and the skill taxonomy mean tool
input schemas carry enum-typed parameters"* — as a deprecation hazard. It is also a trimming hazard,
and now a demonstrated one.

**The analyzer never mentioned it.** The archived `trim-warnings.txt` from the same run contains
exactly two diagnostics, both `IL2026`, both EF Core:
...
So static analysis reported EF Core and stayed silent about the thing that actually broke, while the
runtime failed on a type no diagnostic named. That is ADR-0002's central lesson —
*"static analysis and runtime disagreed in direction"* — reproduced independently, on a different
mechanism, two phases later. It is the strongest argument in this document for why the functional
gate is not optional and why a size number is not a result.

One mercy: it fails **loudly**. `Hosting failed to start` with a stack trace is a far better outcome
than ADR-0002's silently empty tool set, and the gate would have caught either.
...
- **Trimming stays off.** Not deferred pending tuning — the trimmed host does not start. Reversing
  ...
  functional gate rather than against the analyzer, which did not see this.
